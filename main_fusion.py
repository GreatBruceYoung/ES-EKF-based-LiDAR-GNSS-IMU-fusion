...
#### 4. Measurement Update Using ES-EKF #####################################################################
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    for i in range(len(gnss.t)):
        if abs(gnss.t[i] - imu_f.t[k]) < 0.01:
            ...
            p_est[k], v_est[k], q_est[k], p_cov[k] = measurement_update(h_jac,var_gnss, p_cov[k], gnss.data[i],
                                                                p_est[k], v_est[k], q_est[k]) 

    for i in range(len(lidar.t)):
        if abs(lidar.t[i] - imu_f.t[k]) < 0.01:
            #print("hey, a new lidar measurement")
            p_est[k], v_est[k], q_est[k], p_cov[k] = measurement_update(h_jac,var_lidar, p_cov[k], lidar.data[i],
                                                                p_est[k], v_est[k], q_est[k])
...
# error=rmse_calculator(gt.p,p_est)
# print("Error:",error)
...
error_fig.suptitle('Error Plotting')
num_gt = gt.p.shape[0]
p_est_euler = []
p_cov_euler_std = []
...
titles = ['East', 'North', 'Up', 'Roll', 'Pitch', 'Yaw']
for i in range(3):
...
with open('results/pt1_submission.txt', 'w') as file:
    file.write(p1_str)
...
# with open('results/pt2_submission.txt', 'w') as file:
#     file.write(p2_str)
...
# with open('results/pt3_submission.txt', 'w') as file:
#     file.write(p3_str)